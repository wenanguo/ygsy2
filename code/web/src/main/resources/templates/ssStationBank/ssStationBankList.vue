...
<script>
    ...
    import { getSsStationBankList, saveSsStationBank, delSsStationBank, batchDelSsStationBank } from '@/api/ssStationBank'
    import EditForm from './ssStationBankForm'
        ...
        data () {
            ...
            return {
                ...
                loadData: parameter => {
                    ...
                    return getSsStationBankList(requestParameters)
                        .then(res => {
                            return res.result
                        })
                },
        ...
        methods: {
            ...
            handleOk () {
                ...
                form.validateFields((errors, values) => {
                    if (!errors) {
                         // 日期格式化
                            values.createdTime = moment(values.createdTime).format('YYYY-MM-DD HH:mm:ss')
                            values.updatedTime = moment(values.updatedTime).format('YYYY-MM-DD HH:mm:ss')


                        if (values.id > 0) {
                            // 修改 e.g.

                            saveSsStationBank(values).then(res => {
                                this.visible = false
            ...
            handleBatchDel () {
                batchDelSsStationBank(this.selectedRowKeys).then(res => {
                    this.confirmLoading = false
                    // 刷新表格
                    this.$refs.table.refresh()
            ...
            handleDel (record) {
                if (record.id > 0) {
                    // 修改 e.g.
                    delSsStationBank(record).then(res => {
                        this.confirmLoading = false
                        // 刷新表格
                        this.$refs.table.refresh()
