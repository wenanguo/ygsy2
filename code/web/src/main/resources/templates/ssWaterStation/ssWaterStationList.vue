...
<script>
    ...
    import { getSsWaterStationList, saveSsWaterStation, delSsWaterStation, batchDelSsWaterStation } from '@/api/ssWaterStation'
    import EditForm from './ssWaterStationForm'
    ...
    const columns = [
        {
            ...
            title: '可用余额',
            sorter: true,
            dataIndex: 'availableBalanavailableBalance'
        },        {
            title: '冻结余额',
            sorter: true,
        ...
        data () {
            ...
            return {
                ...
                loadData: parameter => {
                    ...
                    return getSsWaterStationList(requestParameters)
                        .then(res => {
                            return res.result
                        })
                },
        ...
        methods: {
            ...
            handleOk () {
                ...
                form.validateFields((errors, values) => {
                    if (!errors) {
                         // 日期格式化
                            values.city = moment(values.city).format('YYYY-MM-DD HH:mm:ss')
                        ...
                        if (values.id > 0) {
                            ...
                            saveSsWaterStation(values).then(res => {
                                this.visible = false
            ...
            handleBatchDel () {
                batchDelSsWaterStation(this.selectedRowKeys).then(res => {
                    this.confirmLoading = false
                    // 刷新表格
                    this.$refs.table.refresh()
            ...
            handleDel (record) {
                if (record.id > 0) {
                    // 修改 e.g.
                    delSsWaterStation(record).then(res => {
                        this.confirmLoading = false
                        // 刷新表格
                        this.$refs.table.refresh()
