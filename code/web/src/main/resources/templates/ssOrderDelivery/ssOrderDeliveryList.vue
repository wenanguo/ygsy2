...
<script>
    ...
    import { getSsOrderDeliveryList, saveSsOrderDelivery, delSsOrderDelivery, batchDelSsOrderDelivery } from '@/api/ssOrderDelivery'
    import EditForm from './ssOrderDeliveryForm'
    ...
    const columns = [
        {
            ...
            title: '配送状态 0-已接单，1-配送中，2-已配送，3-已取消',
            sorter: true,
            width: '100px',
        ...
        data () {
            ...
            return {
                ...
                loadData: parameter => {
                    ...
                    return getSsOrderDeliveryList(requestParameters)
                        .then(res => {
                            return res.result
                        })
                },
        ...
        methods: {
            ...
            handleOk () {
                ...
                form.validateFields((errors, values) => {
                    if (!errors) {
                         // 日期格式化
                            values.receiveTime = moment(values.receiveTime).format('YYYY-MM-DD HH:mm:ss')
                            values.finishTime = moment(values.finishTime).format('YYYY-MM-DD HH:mm:ss')
                            values.deleveryTime = moment(values.deleveryTime).format('YYYY-MM-DD HH:mm:ss')


                        if (values.id > 0) {
                            // 修改 e.g.

                            saveSsOrderDelivery(values).then(res => {
                                this.visible = false
            ...
            handleBatchDel () {
                batchDelSsOrderDelivery(this.selectedRowKeys).then(res => {
                    this.confirmLoading = false
                    // 刷新表格
                    this.$refs.table.refresh()
            ...
            handleDel (record) {
                if (record.id > 0) {
                    // 修改 e.g.
                    delSsOrderDelivery(record).then(res => {
                        this.confirmLoading = false
                        // 刷新表格
                        this.$refs.table.refresh()
