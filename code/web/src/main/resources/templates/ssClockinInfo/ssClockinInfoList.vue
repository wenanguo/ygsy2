...
<script>
    ...
    import { getSsClockinInfoList, saveSsClockinInfo, delSsClockinInfo, batchDelSsClockinInfo } from '@/api/ssClockinInfo'
    import EditForm from './ssClockinInfoForm'
        ...
        data () {
            ...
            return {
                ...
                loadData: parameter => {
                    ...
                    return getSsClockinInfoList(requestParameters)
                        .then(res => {
                            return res.result
                        })
                },
        ...
        methods: {
            ...
            handleOk () {
                ...
                form.validateFields((errors, values) => {
                    if (!errors) {
                         // 日期格式化
                            values.inTime = moment(values.inTime).format('YYYY-MM-DD HH:mm:ss')
                            values.outTime = moment(values.outTime).format('YYYY-MM-DD HH:mm:ss')


                        if (values.id > 0) {
                            // 修改 e.g.

                            saveSsClockinInfo(values).then(res => {
                                this.visible = false
            ...
            handleBatchDel () {
                batchDelSsClockinInfo(this.selectedRowKeys).then(res => {
                    this.confirmLoading = false
                    // 刷新表格
                    this.$refs.table.refresh()
            ...
            handleDel (record) {
                if (record.id > 0) {
                    // 修改 e.g.
                    delSsClockinInfo(record).then(res => {
                        this.confirmLoading = false
                        // 刷新表格
                        this.$refs.table.refresh()
