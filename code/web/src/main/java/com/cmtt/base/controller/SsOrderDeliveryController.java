 ...
 */
@RestController
@RequestMapping("/api/base/ss-order-delivery")
@Api(tags = "配送员接单相关")
public class SsOrderDeliveryController {

    private final Logger logger = LoggerFactory.getLogger(SsOrderDeliveryController.class);
    ...
    public R add(@RequestBody @Validated({GroupAdd.class}) OrderDeliveryInputParam param, Principal principal) {
        try {
            ...
            ssOrderDelivery.setDeliveryType(1);//设置为个人配送
            ssOrderDelivery.setReceiveTime(LocalDateTime.now());
            ssOrderDeliveryService.save(ssOrderDelivery);//保存接单配送信息
            ssOrderDeliveryService.updateOrderStatus(2, param.getOrderNumber());//订单更新为已接单
    ...
    public R delivery(@RequestBody @Validated({GroupEdit.class}) OrderDeliveryInputParam param, Principal principal) {
        try {
            ...
            ssOrderDeliveryService.updateOrderStatus(3, param.getOrderNumber());//订单更新为配送中
    ...
    public R finish(@RequestBody @Validated({GroupEdit.class}) OrderDeliveryInputParam param, Principal principal) {
        try {
            ...
            ssOrderDeliveryService.updateOrderStatus(4, param.getOrderNumber());//订单更新为已完成
