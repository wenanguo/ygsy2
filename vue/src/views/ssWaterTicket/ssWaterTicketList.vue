      ...
      >
        <span slot="serial" slot-scope="text, record, index">
         ...
         <span slot="pic" slot-scope="text, record">
          <img style="width:50px;heigth:50px" :src="record.ticketImg" />
        </span>
...
<script>
    ...
    import { getSsWaterTicketList, saveSsWaterTicket, delSsWaterTicket, batchDelSsWaterTicket } from '@/api/ssWaterTicket'
    import EditForm from './ssWaterTicketForm'
    ...
    export default {
        ...
        data () {
            ...
            return {
                ...
                loadData: parameter => {
                    ...
                    return getSsWaterTicketList(requestParameters)
                        .then(res => {
                            return res.result
                        })
                },
        ...
        methods: {
                ...
                form.validateFields((errors, values) => {
                    if (!errors) {
                        ...
                        if (values.id > 0) {
                            ...
                            saveSsWaterTicket(values).then(res => {
                                this.visible = false
            ...
            handleBatchDel () {
                batchDelSsWaterTicket(this.selectedRowKeys).then(res => {
                    this.confirmLoading = false
                    // 刷新表格
                    this.$refs.table.refresh()
            ...
            handleDel (record) {
                if (record.id > 0) {
                    // 修改 e.g.
                    delSsWaterTicket(record).then(res => {
                        this.confirmLoading = false
                        // 刷新表格
                        this.$refs.table.refresh()
