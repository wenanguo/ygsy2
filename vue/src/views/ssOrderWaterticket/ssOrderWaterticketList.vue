...
<script>
    ...
    import { getSsOrderWaterticketList, saveSsOrderWaterticket, delSsOrderWaterticket, batchDelSsOrderWaterticket } from '@/api/ssOrderWaterticket'
    import EditForm from './ssOrderWaterticketForm'
        ...
        data () {
            ...
            return {
                ...
                loadData: parameter => {
                    ...
                    return getSsOrderWaterticketList(requestParameters)
                        .then(res => {
                            return res.result
                        })
                },
        ...
        methods: {
            ...
            handleOk () {
                ...
                form.validateFields((errors, values) => {
                    if (!errors) {
                         // 日期格式化
                            values.ticketNumber = moment(values.ticketNumber).format('YYYY-MM-DD HH:mm:ss')


                        if (values.id > 0) {
                            // 修改 e.g.

                            saveSsOrderWaterticket(values).then(res => {
                                this.visible = false
            ...
            handleBatchDel () {
                batchDelSsOrderWaterticket(this.selectedRowKeys).then(res => {
                    this.confirmLoading = false
                    // 刷新表格
                    this.$refs.table.refresh()
            ...
            handleDel (record) {
                if (record.id > 0) {
                    // 修改 e.g.
                    delSsOrderWaterticket(record).then(res => {
                        this.confirmLoading = false
                        // 刷新表格
                        this.$refs.table.refresh()
