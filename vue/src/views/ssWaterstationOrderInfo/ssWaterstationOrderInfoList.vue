<template>
    <page-header-wrapper>
                        ...
                        <a-col :md="8" :sm="24">
                            <a-form-item label="使用状态">
                                <a-select v-model="queryParam.orderStatus" placeholder="请选择" default-value="0">
                                    ...
                                    <a-select-option :value="2">已领单</a-select-option>
        ...
        <span slot="orderDetail" slot-scope="text, record">
          <template>
            <a @click="handleLink(text,record,'orderDetail')">订单明细</a>
          </template>          
        </span>
...
<script>
    ...
    import { getSsWaterstationOrderInfoList, saveSsWaterstationOrderInfo, delSsWaterstationOrderInfo, batchDelSsWaterstationOrderInfo } from '@/api/ssWaterstationOrderInfo'
    import EditForm from './ssWaterstationOrderInfoForm'
    ...
    const columns = [
        ...
        {
            title: '订单状态 1-待分配，2-配送中，3-已完成，4-已取消',
            sorter: true,
        ...
        // },        
        {
            ...
            title: '支付状态：201-未支付,202-支付中,203-支付成功,204-支付失败',
            sorter: true,
            dataIndex: 'payStatus'
        },        {
            title: '支付方式 1-水票，2-微信，3-水到付款',
            sorter: true,
            dataIndex: 'payWay'
        },
         {
        ...
        data () {
            ...
            return {
                ...
                loadData: parameter => {
                    ...
                    return getSsWaterstationOrderInfoList(requestParameters)
                        .then(res => {
                            return res.result
                        })
                },
        ...
        methods: {
            ...
            handleLink (text,record,whats) {
             ...
             //  alert(record.customerId);
              if(whats=='orderDetail'){
                //   alert(record.customerId);
                  this.$router.push({ name: 'SsWaterstationOrderDetail', params: {customerId:record.customerId}})
              }
            //  alert(record.customerId);
                ...
                form.validateFields((errors, values) => {
                    if (!errors) {
                        ...
                        if (values.id > 0) {
                            ...
                            saveSsWaterstationOrderInfo(values).then(res => {
                                this.visible = false
            ...
            handleBatchDel () {
                batchDelSsWaterstationOrderInfo(this.selectedRowKeys).then(res => {
                    this.confirmLoading = false
                    // 刷新表格
                    this.$refs.table.refresh()
            ...
            handleDel (record) {
                if (record.id > 0) {
                    // 修改 e.g.
                    delSsWaterstationOrderInfo(record).then(res => {
                        this.confirmLoading = false
                        // 刷新表格
                        this.$refs.table.refresh()
