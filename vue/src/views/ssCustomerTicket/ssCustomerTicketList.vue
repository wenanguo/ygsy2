          ...
          <a-row :gutter="48">
            <a-col :md="8" :sm="24">
              <a-form-item label="名称">
                <a-input v-model="queryParam.ticketName" placeholder=""/>
              </a-form-item>
            </a-col>
            <a-col :md="8" :sm="24">
...
<script>
    ...
    import { getSsCustomerTicketList, saveSsCustomerTicket, delSsCustomerTicket, batchDelSsCustomerTicket } from '@/api/ssCustomerTicket'
    import EditForm from './ssCustomerTicketForm'
        ...
        data () {
            ...
            return {
                ...
                loadData: parameter => {
                    ...
                    return getSsCustomerTicketList(requestParameters)
                        .then(res => {
                            return res.result
                        })
                },
        ...
        methods: {
            ...
            handleOk () {
                ...
                form.validateFields((errors, values) => {
                    if (!errors) {
                         // 日期格式化
                            values.useTime = moment(values.useTime).format('YYYY-MM-DD HH:mm:ss')


                        if (values.id > 0) {
                            // 修改 e.g.

                            saveSsCustomerTicket(values).then(res => {
                                this.visible = false
            ...
            handleBatchDel () {
                batchDelSsCustomerTicket(this.selectedRowKeys).then(res => {
                    this.confirmLoading = false
                    // 刷新表格
                    this.$refs.table.refresh()
            ...
            handleDel (record) {
                if (record.id > 0) {
                    // 修改 e.g.
                    delSsCustomerTicket(record).then(res => {
                        this.confirmLoading = false
                        // 刷新表格
                        this.$refs.table.refresh()
