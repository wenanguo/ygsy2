    ...
    <a-card :bordered="false">
      ...
      >
        ...
        <span slot="station" slot-scope="text">
          {{ handleStationName(text) }}
        </span>
        <span slot="sexText" slot-scope="text">
          {{ text==1 ? '男' : '女' }}
        </span>
        <span slot="typeText" slot-scope="text">
          {{ text==3 ? '水站配送员' : '水厂配送员' }}
        </span>
...
<script>
    ...
    import { getSsDeliveryManList, saveSsDeliveryMan, delSsDeliveryMan, batchDelSsDeliveryMan } from '@/api/ssDeliveryMan'
    import EditForm from './ssDeliveryManForm'
    ...
    const columns = [
        {
            ...
            sorter: true,
            dataIndex: 'sex',
            scopedSlots: { customRender: 'sexText' }
        },        {
            title: '状态',
            sorter: true,
            width: '100px',
    ...
    export default {
        ...
        data () {
            ...
            return {
                ...
                loadData: parameter => {
                    ...
                    return getSsDeliveryManList(requestParameters)
                        .then(res => {
                            return res.result
                        })
                },
        ...
        methods: {
            ...
            handleEdit (record) {
                this.title = '修改'
                this.visible = true
                record.stationId = parseInt(record.stationId)
                this.mdl = { ...record }
            },
            handleOk () {
                ...
                form.validateFields((errors, values) => {
                    if (!errors) {
                        ...
                        if (values.id > 0) {
                            ...
                            saveSsDeliveryMan(values).then(res => {
                                this.visible = false
            ...
            handleBatchDel () {
                batchDelSsDeliveryMan(this.selectedRowKeys).then(res => {
                    this.confirmLoading = false
                    // 刷新表格
                    this.$refs.table.refresh()
            ...
            handleDel (record) {
                if (record.id > 0) {
                    // 修改 e.g.
                    delSsDeliveryMan(record).then(res => {
                        this.confirmLoading = false
                        // 刷新表格
                        this.$refs.table.refresh()
            ...
            handleStationName(id) {
                for (var i = 0; i < this.stationList.length; i++) {
                    if(id == this.stationList[i].id){
                      var s = this.stationList[i].station_name
                      break
                    }
                }
                return s
